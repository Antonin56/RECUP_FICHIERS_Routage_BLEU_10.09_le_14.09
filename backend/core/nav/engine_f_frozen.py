"""SignalMar — MOTEUR F GELÉ (27/08/2026, ordre armateur — MAINTENANCE CRITIQUE).

⛔ RÈGLE ABSOLUE : CE FICHIER NE DOIT PLUS JAMAIS ÊTRE MODIFIÉ. ⛔

Copie intégrale, à l'identique, de la logique du Moteur F validée par le
support au 27/08/2026 : ``signalmar_v6/sidefix.py`` (section 1) +
``signalmar_v6/__init__.py`` (section 2, classe renommée ``EngineFFrozen``,
algo ``signalmar.f_frozen``). ``routers/routing.py`` pointe EXCLUSIVEMENT
ici pour le moteur ``engine_f`` — le Moteur F est une référence immuable.

Dépendances importées : UNIQUEMENT des modules eux-mêmes figés (Moteurs A-E :
signalmar_v1..v5, standoff v2, audit v4) ou d'infrastructure (bathy,
seamarks). Toute évolution future se fait sur le Moteur I
(``core/nav/engine_i.py``) — jamais ici.
"""
...
class EngineFFrozen(SignalmarV5):
    id = "signalmar.f_frozen"
    version = "6.1.0"
    description = (
