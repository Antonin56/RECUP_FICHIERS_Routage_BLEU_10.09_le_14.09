...
import { NM_IN_M, type VoiceSettings } from "@/src/lib/voice-alerts";
...
export function AlertSettingsPanel({
  ...
  const commitNav = useCallback((m: number) => {
    // 17/07/2026 (v2 cône coloré) — si l'utilisateur diminue la longueur du
    // cône EN DESSOUS de sa distance d'alerte, on re-clampe automatique-
    // ment la distance d'alerte au nouveau max (invariant : alertDist ≤ zoneNav).
    const patch: Partial<VoiceSettings> = { zoneNavM: m };
    const curAlert = settings.alertDistanceNavM ?? m;
    if (curAlert > m) {
      patch.alertDistanceNavM = m;
      showToast("info", "Distance d'alerte ramenée à la longueur du cône.");
    }
    void update(patch);
  }, [update, settings.alertDistanceNavM]);

  // 17/07/2026 — Commit de la distance d'alerte NAV. Bornée à [100 m, zoneNavM].
  const commitAlertDistNav = useCallback((m: number) => {
    const zNav = settings.zoneNavM ?? 5 * NM_IN_M;
    const clamped = Math.min(zNav, Math.max(100, m));
    void update({ alertDistanceNavM: clamped });
  }, [update, settings.zoneNavM]);
  ...
  if (variant === "alert") {
    ...
    const zNavM = settings.zoneNavM ?? 5 * NM_IN_M;
    const alertM = Math.min(zNavM, settings.alertDistanceNavM ?? zNavM);
    return (
      <View style={styles.root}>
        <ZoneField
          ...
          valueM={isNav ? zNavM : (settings.zoneVigieM ?? 2 * NM_IN_M)}
        ...
        {/* 17/07/2026 (v2 cône coloré) — Distance de déclenchement d'alerte
            EN MODE NAVIGATION UNIQUEMENT. Bornée par la longueur du cône. */}
        {isNav && (
          <>
            <View style={styles.zoneGap} />
            <ZoneField
              icon="alert-circle"
              label="Distance d'alerte en mode navigation"
              hint="À l'intérieur de cette distance le cône devient rouge et l'alerte sonore se déclenche. Au-delà (mais dans le cône) : cône orange, pas de son."
              valueM={alertM}
              onCommitM={commitAlertDistNav}
              testIDPrefix="alert-dist-nav"
              showHint={false}
              sliderMaxKm={Math.max(0.5, zNavM / 1000)}
              inputMaxKm={Math.max(0.5, zNavM / 1000)}
            />
          </>
        )}
  ...
  return (
    <View style={styles.root}>
      ...
      />
      {/* 17/07/2026 (v2 cône coloré) — Distance d'alerte NAV, bornée par
          la longueur du cône. Rendu uniquement sur la page Réglages
          « pleine » (variant='full') ; la popup rapide reçoit son propre
          slider ci-dessus via variant='alert'. */}
      <View style={styles.zoneGap} />
      <ZoneField
        icon="alert-circle"
        label="Distance d'alerte en mode navigation"
        hint="À l'intérieur de cette distance le cône devient rouge et l'alerte sonore se déclenche. Au-delà (mais dans le cône) : cône orange, pas de son."
        valueM={Math.min(
          settings.zoneNavM ?? 5 * NM_IN_M,
          settings.alertDistanceNavM ?? (settings.zoneNavM ?? 5 * NM_IN_M),
        )}
        onCommitM={commitAlertDistNav}
        testIDPrefix="alert-dist-nav"
        showHint={full}
        sliderMaxKm={Math.max(0.5, (settings.zoneNavM ?? 5 * NM_IN_M) / 1000)}
        inputMaxKm={Math.max(0.5, (settings.zoneNavM ?? 5 * NM_IN_M) / 1000)}
      />
