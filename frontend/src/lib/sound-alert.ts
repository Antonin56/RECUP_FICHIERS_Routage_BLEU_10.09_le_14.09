...
export function useSoundAlertWatcher(args: SoundAlertArgs) {
  ...
  useEffect(() => {
    ...
    // La distance d'alerte en NAVIGATION peut désormais être différente
    // (≤) de la longueur du cône. Fallback strict = zoneNavM ⇒ AUCUNE
    // régression sonore pour les utilisateurs existants (voir migration
    // voice-settings.ts). Le CÔNE reste à zoneNavM (awareness), c'est le
    // SEUIL SONORE qui devient alertDistanceNavM.
    const alertDistNavM = Math.min(
      zoneNavM,
      Math.max(100, voiceSettings.alertDistanceNavM ?? zoneNavM),
    );
    const now = Date.now();
    const coneReady =
      navMode &&
    ...
    for (const r of reports) {
      ...
      if (!navMode) {
        ...
        // 17/07/2026 — le SEUIL SONORE est désormais `alertDistNavM`
        // (≤ zoneNavM = longueur du cône). Un signalement DANS le cône
        // mais au-delà de alertDistNavM colore le cône en ORANGE (visuel)
        // sans faire sonner — l'orange est géré par MarineMap indépendam-
        // ment. Ici on ne parle QUE du déclenchement audio (rouge).
        const inConeShape = d <= zoneNavM && isInCone(
          userLoc.lat, userLoc.lng, userHeading as number,
          coneHalfAngleDeg as number, r.lat, r.lng, d,
        );
        const inAlertZone = inConeShape && d <= alertDistNavM;
        if (inAlertZone) {
        ...
        // Nav manuel sans cap connu → repli 360° sur alertDistNavM
        // (sécurité — on prend la ZONE D'ALERTE, pas le cône entier).
        inside = d <= alertDistNavM;
      }

      // ── Ré-armement à la sortie franche du périmètre ──
      const clearlyOutside = navMode
        ? d > alertDistNavM * EXIT_HYSTERESIS || (coneReady && !coneSince.has(r.id) && !inside && d > zoneVigieM * EXIT_HYSTERESIS)
